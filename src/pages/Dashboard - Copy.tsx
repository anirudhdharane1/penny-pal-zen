...
import { TrendingUp, TrendingDown, Wallet, Target, DollarSign } from "lucide-react";
import DashboardLayout from "@/components/DashboardLayout";
import { getToken } from "@/lib/api"; // use your existing token helper

const API_INCOME_TOTAL = "http://localhost:5000/api/income/total";
const API_EXPENSE_TOTAL = "http://localhost:5000/api/budgets/total-spent";
const API_INCOME_ALL = "http://localhost:5000/api/income"; // optional for chart
const API_BUDGET_ALL = "http://localhost:5000/api/budgets"; // optional for chart
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Optionally fetch detailed data for charts
        const [allIncomeRes, allBudgetRes] = await Promise.all([
          fetch(API_INCOME_ALL, { headers: { Authorization: `Bearer ${getToken()}` } }),
          fetch(API_BUDGET_ALL, { headers: { Authorization: `Bearer ${getToken()}` } }),
        ]);
        ...
        setIncomes(
          incomeList.map((inc: any) => ({
            source: inc.source || inc.category || "Unknown",
            amount: inc.amount || 0,
            color: "#6366F1",
          }))
        );
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-xl font-semibold">Monthly Savings Goal</h3>
              <p className="text-muted-foreground">
                Target: ₹{profile?.monthly_saving_goal?.toLocaleString() || 0}
              </p>
            </div>
            <Target className="w-8 h-8 text-success" />
          </div>
          ...
          <div className="mt-2 text-right">
            ...
            <span className="text-muted-foreground">
              {" "}
              / ₹{profile?.monthly_saving_goal?.toLocaleString() || 0}
            </span>
          </div>
        </Card>
      </div>
